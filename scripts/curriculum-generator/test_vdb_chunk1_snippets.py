...
json_files = [
    os.path.join(base_dir, f"vdb_ch{ch}_data.json") for ch in [1, 2, 3, 4]
]
...
print("=" * 70)
print("TEST RUNNER: Verifikasi Eksekusi Mandiri Snippet Kode Chunk 1 (Bab 1-4)")
print("=" * 70)
...
for file_idx, json_file in enumerate(json_files, 1):
    ...
    print(f"\nMemverifikasi Bab {file_idx} ({len(subchapters)} subbab)...")
    for sub in subchapters:
        total_snippets += 1
        sub_id = sub["id"]
        sub_title = sub["title"]
        
        # Ekstrak code dan expected output baik dari content dict maupun codeExamples
...
if failed_snippets:
    ...
    print("[SUKSES] Semua 40 snippet Python teruji mandiri dan deterministik.")
