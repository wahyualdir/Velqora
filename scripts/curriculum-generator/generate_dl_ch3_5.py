# scripts/curriculum-generator/generate_dl_ch3_5.py
"""
Script to generate dl-data-ch3-5.ts with 30 comprehensive subchapters across Chapters 3, 4, and 5.
All content adheres to rigorous university-level academic standards (MIT 6.S191, Stanford CS230, Goodfellow et al. 2016).
"""
import json
import os
...
chapters = [
    ...
    },
    {
        ...
        "subchapters": ch5_subchapters
    }
]

def format_subchapter(sub):
    # Escape backticks in concept, formula, code, expectedOutput, codeExp, pitfalls
    def esc(text):
        if not text:
            return ""
        t = text.replace('`', '\\`')
    ...
    return f"""      {{
        ...
        title: "{sub['title']}",
        desc: "{sub['desc']}",
        concept: `{esc(sub['concept'])}`,
        formula: `{esc(sub['formula'])}`,
        code: `{esc(sub['code'])}`,
        codeExp: `{esc(sub['codeExp'])}`,
        expectedOutput: `{esc(sub['expectedOutput'])}`,
        pitfalls: `{esc(sub['pitfalls'])}`,
        refUrl: "{sub['refUrl']}"
      }}"""

def format_chapter(ch):
    subs_str = ",\n".join([format_subchapter(s) for s in ch['subchapters']])
    return f"""  {{
    id: "{ch['id']}",
    title: "{ch['title']}",
    desc: "{ch['desc']}",
...
def generate_ts():
    chs_str = ",\n".join([format_chapter(c) for c in chapters])
    ts_content = f"""// scripts/curriculum-generator/dl-data-ch3-5.ts
...
"""
    with open("scripts/curriculum-generator/dl-data-ch3-5.ts", "w", encoding="utf-8") as f:
        f.write(ts_content)
    print(f"Generated dl-data-ch3-5.ts successfully! Chapters: {len(chapters)}, Subchapters: {sum(len(c['subchapters']) for c in chapters)}")
