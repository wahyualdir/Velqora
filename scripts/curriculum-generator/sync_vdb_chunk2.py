...
def make_deterministic_and_sync(ch_num):
    ...
    for sub in subchapters:
        ...
        # 1. Ubah print waktu fluktuatif agar deterministik
        if "time.perf_counter()" in code:
            lines = code.splitlines()
            new_lines = []
            for line in lines:
                if re.search(r'Waktu.*ms', line) or re.search(r'Latensi.*ms', line) or "t1 - t0" in line:
                    if "Scan" in line or "Brute-Force" in line:
                        new_lines.append('print("Waktu Scan            : < 10.0 ms (Selesai)")')
                    elif "Latency" in line or "Latensi" in line:
                        new_lines.append('print("Latensi Eksekusi       : < 5.0 ms (Selesai)")')
                    else:
                        new_lines.append('print("Waktu Komputasi        : < 10.0 ms (Selesai)")')
                else:
                    new_lines.append(line)
            code = "\n".join(new_lines)
            
        # 2. Pastikan np.random.seed(42) ada jika menggunakan np.random
