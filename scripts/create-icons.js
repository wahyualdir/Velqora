...
const svgTarget = path.join(iconsDir, 'icon.svg');
fs.copyFileSync(svgSource, svgTarget);

// Create SVG-based 192 and 512 SVGs
const svg192 = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 192 192" width="192" height="192" fill="none">
  ...
  <rect x="1" y="1" width="190" height="190" rx="41" stroke="#334155" stroke-width="2"/>
...
const svg512 = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512" width="512" height="512" fill="none">
  ...
  <rect x="2" y="2" width="508" height="508" rx="110" stroke="#334155" stroke-width="4"/>
...
// Copy or create fallback PNG files from existing public/logo.jpg or logo-banner.png
const logoJpg = path.join(__dirname, '..', 'public', 'logo.jpg');
if (fs.existsSync(logoJpg)) {
  fs.copyFileSync(logoJpg, path.join(iconsDir, 'icon-192.png'));
  fs.copyFileSync(logoJpg, path.join(iconsDir, 'icon-512.png'));
  fs.copyFileSync(logoJpg, path.join(iconsDir, 'icon-maskable-512.png'));
}
