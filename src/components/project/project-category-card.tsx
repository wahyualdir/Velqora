...
export function ProjectCategoryCard({ category, onClick }: ProjectCategoryCardProps) {
  const IconComponent = getCategoryIconComponent(category.icon);

  return (
    <div
      onClick={onClick}
      className="group block p-4 sm:p-5 vt-window bg-[#FFFFFF] dark:bg-[#18181B] hover:bg-[#FAF8F5] dark:hover:bg-[#202024] transition-all relative overflow-hidden border border-border cursor-pointer"
      style={{
        borderTopWidth: "3px",
    ...
    >
      ...
      <div className="mt-4 pt-3 border-t border-border/50 flex items-center justify-between text-xs font-mono text-text-tertiary group-hover:text-brand-600 dark:group-hover:text-brand-400">
        <span className="font-medium">Lihat Proyek</span>
