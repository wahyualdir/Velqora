...
/**
 * Normalizes any category string, subcategory title, or key into a known icon key
 */
export function resolveIconKey(rawName?: string | null): string {
  if (!rawName) return "code";
  const str = rawName.toLowerCase().trim();

  // Languages
  if (str.includes("python") || str === "py") return "python";
  if (str === "javascript" || str === "js" || str.includes("javascript")) return "javascript";
  if (str === "typescript" || str === "ts" || str.includes("typescript")) return "typescript";
  if (str === "c++" || str === "cpp" || str.includes("cplusplus")) return "cpp";
  if (str === "c#" || str === "csharp" || str.includes("c-sharp")) return "csharp";
  if (str === "c" || str.startsWith("c ") || str === "c language" || str === "bahasa c") return "c";
  if (str === "java" || (str.includes("java") && !str.includes("javascript") && !str.includes("script"))) return "java";
  if (str.includes("kotlin")) return "kotlin";
  if (str.includes("swift") || str.includes("ios")) return "swift";
  if (str.includes("golang") || str === "go" || str.startsWith("go ")) return "golang";
  ...
  // Frameworks & Web
  if (str.includes("next.js") || str.includes("nextjs") || str === "next") return "nextjs";
  if (str.includes("react native") || str.includes("react")) return "react";
  if (str.includes("vue")) return "vue";
  if (str.includes("angular")) return "angular";
  if (str.includes("svelte")) return "svelte";
  if (str.includes("node") || str.includes("express")) return "node";
  if (str.includes("laravel")) return "laravel";
  if (str.includes("django")) return "django";
  if (str.includes("flask") || str.includes("fastapi")) return "python";
  if (str.includes("spring") || str.includes("springboot")) return "spring";
  if (str.includes("flutter")) return "flutter";
  if (str.includes("android")) return "android";
  if (str.includes("apple") || str.includes("ios")) return "apple";

  // Databases
  if (str.includes("mysql")) return "mysql";
  if (str.includes("postgres") || str.includes("postgresql")) return "postgresql";
  if (str.includes("mongo") || str.includes("mongodb")) return "mongodb";
  if (str.includes("redis")) return "redis";
  if (str.includes("sqlite")) return "sqlite";
  if (str.includes("sql") || str.includes("database") || str.includes("basis data")) return "sql";

  // DevOps & Cloud
  if (str.includes("docker")) return "docker";
  if (str.includes("kubernetes") || str.includes("k8s")) return "kubernetes";
  if (str.includes("linux") || str.includes("ubuntu")) return "linux";
  if (str.includes("github")) return "github";
  if (str.includes("git") || str.includes("version control") || str.includes("ci/cd")) return "git";
  if (str.includes("aws") || str.includes("amazon")) return "aws";
  if (str.includes("google cloud") || str.includes("gcp")) return "gcp";
  if (str.includes("azure")) return "azure";
  if (str.includes("nginx")) return "nginx";

  // Design & UI/UX
  if (str.includes("figma") || str.includes("ui") || str.includes("ux") || str.includes("design") || str.includes("wireframing") || str.includes("prototyping") || str.includes("typography")) return "figma";
  if (str.includes("excel")) return "excel";

  // AI & Data
  if (str.includes("generative ai") || str.includes("genai") || str.includes("llm") || str.includes("rag") || str.includes("prompt") || str.includes("multimodal")) return "generative_ai";
  if (str.includes("deep learning") || str.includes("neural") || str.includes("cnn") || str.includes("rnn")) return "deep_learning";
  if (str.includes("nlp") || str.includes("natural language") || str.includes("tokenization") || str.includes("bert") || str.includes("word2vec") || str.includes("text")) return "nlp";
  if (str.includes("vision") || str.includes("opencv") || str.includes("yolo") || str.includes("image") || str.includes("face")) return "computer_vision";
  if (str.includes("machine learning") || str.includes("ai") || str.includes("artificial intelligence")) return "machine_learning";
  if (str.includes("speech") || str.includes("audio")) return "speech";
  if (str.includes("data science") || str.includes("data analytics") || str.includes("analytics") || str.includes("bi") || str.includes("dashboard") || str.includes("kpi")) return "data_science";

  // Algorithms & Math
  if (str.includes("algorithm") || str.includes("algoritma") || str.includes("struktur data") || str.includes("tree") || str.includes("graph") || str.includes("sorting") || str.includes("searching") || str.includes("dynamic programming") || str.includes("stack") || str.includes("queue") || str.includes("array") || str.includes("hash")) return "algorithm";
  if (str.includes("matematika") || str.includes("math") || str.includes("calculus") || str.includes("algebra") || str.includes("statistics") || str.includes("probability") || str.includes("discrete")) return "math";

  // Systems & Hardware
  if (str.includes("sistem komputer") || str.includes("computer") || str.includes("operating system") || str.includes("cpu") || str.includes("networking") || str.includes("memory") || str.includes("hardware")) return "computer_systems";
  if (str.includes("security") || str.includes("keamanan") || str.includes("cyber") || str.includes("owasp") || str.includes("encryption") || str.includes("cryptography")) return "security";
...
export function TechIcon({ name, className = "w-5 h-5", size = 20, animate = true, onClick }: TechIconProps) {
  ...
  const wrapIcon = (iconEl: React.ReactNode) => (
    <span
      className={`inline-flex tech-icon-wrapper items-center justify-center shrink-0 ${bouncing ? "tech-icon-bounce" : ""}`}
      onClick={handleClick}
      style={{ cursor: animate || onClick ? "pointer" : "default" }}
    >
      {iconEl}
    </span>
  );
