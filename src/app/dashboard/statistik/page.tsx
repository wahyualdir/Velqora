...
export default function StatistikPage() {
  ...
  const totalMateri = stats?.totalMateri || 0;
  ...
  if (loading) {
    return (
      <div className="page-container space-y-6 sm:space-y-8 animate-pulse pb-12">
        ...
        <div className="card-grid-stats">
          {[1, 2, 3, 4].map((i) => (
            <div key={i} className="h-36 rounded-2xl bg-surface border border-border p-5" />
          ))}
        </div>
        <div className="grid grid-cols-1 lg:grid-cols-5 gap-4 sm:gap-6">
          <div className="lg:col-span-3 h-80 rounded-2xl bg-surface border border-border" />
          <div className="lg:col-span-2 h-80 rounded-2xl bg-surface border border-border" />
        </div>
      </div>
    );
  }
