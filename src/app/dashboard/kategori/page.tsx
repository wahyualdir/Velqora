...
import { Plus, FolderOpen, Trash2, FolderTree, Sparkles, ArrowRight, BookOpen } from "lucide-react";
    ...
    <div className="page-container space-y-6 sm:space-y-8 animate-fade-in pb-12">
        ...
        actions={
          categories.length === 0 ? (
            <Button
              ...
              loading={submitting}
              variant="outline"
              className="gap-2 border-brand-500/30 text-brand-600 hover:bg-brand-500/10 text-xs h-9"
            >
              <Sparkles className="w-4 h-4 text-brand-600" /> Kategori Bawaan
            </Button>
          ) : undefined
        }
      />
      ...
      <Card className="p-4 sm:p-5 rounded-2xl bg-surface border-border space-y-4">
        <h3 className="text-sm font-semibold text-text-primary flex items-center gap-2 font-display">
          <Plus className="w-4 h-4 text-brand-600" /> Tambah Kategori atau Subkategori
        </h3>
        ...
        <form onSubmit={handleCreate} className="space-y-4">
          ...
          <div className="flex justify-end pt-2 border-t border-border">
            <Button type="submit" loading={submitting} className="gap-1.5 text-xs font-semibold">
              <Plus className="w-4 h-4 mr-1" /> Simpan Kategori
            </Button>
          </div>
        </form>
      </Card>
      ...
      {loading ? (
        ...
        <EmptyState
          ...
          action={
            <Button onClick={handleCreatePresets} className="gap-2">
              <Sparkles className="w-4 h-4" /> Buat Preset Kategori Otomatis
            </Button>
          }
        />
      ) : (
        <div className="space-y-4">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
            <h2 className="text-base font-bold text-text-primary flex items-center gap-2 font-display">
              <FolderTree className="w-5 h-5 text-brand-600" /> Daftar Kategori & Logo Bahasa
            </h2>
          ...
          <div className="space-y-4">
            {topParents.map((parent) => {
              ...
              return (
                <Card key={parent.id} className="p-4 sm:p-5 rounded-2xl bg-surface border-border space-y-3.5 border-l-4" style={{ borderLeftColor: parent.color }}>
                  <div className="flex items-center justify-between">
                    ...
                    <button
                      ...
                      className="p-1.5 rounded-lg text-text-tertiary hover:text-accent-red hover:bg-surface-tertiary transition-colors cursor-pointer"
                    >
                  ...
                  {children.length > 0 && (
                    <div className="pt-3 border-t border-border grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2.5">
                      {children.map((sub) => (
                        <div
                          key={sub.id}
                          className="flex flex-col justify-between p-3 rounded-xl border border-border bg-surface-secondary/80 hover:border-indigo-500/40 hover:bg-surface-tertiary transition-all shadow-sm group"
                        >
                          <div className="flex items-center justify-between gap-2">
                            ...
                            <button
                              ...
                              className="p-1 rounded text-text-tertiary hover:text-accent-red hover:bg-surface transition-colors"
                          ...
                          <Link
                            ...
                            className="mt-2.5 flex items-center justify-between text-[11px] font-semibold text-text-secondary hover:text-text-primary px-2.5 py-1.5 rounded-lg bg-surface hover:bg-surface-secondary border border-border transition-all"
                          >
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-2.5">
                  {categories
                    ...
                    .map((cat) => (
                      <div
                        key={cat.id}
                        className="flex flex-col justify-between p-3 rounded-xl border border-border bg-surface hover:border-indigo-500/40 transition-all shadow-sm group"
                      >
                        <div className="flex items-center justify-between gap-2">
                          <div className="flex items-center gap-2 min-w-0">
                          ...
                          <button
                            ...
                            className="p-1 rounded text-text-tertiary hover:text-accent-red"
                          >
                            <Trash2 className="w-3.5 h-3.5" />
                          </button>
                        </div>
